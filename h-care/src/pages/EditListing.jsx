...
const EditListing = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      if (product) {
        ...
        setYear(product.year_of_manufacture);
        setBrand(product.brand);
        setModel(product.model);
        setLengthVal(product.dimensions?.length || '');
        setWidthVal(product.dimensions?.width || '');
        setHeightVal(product.dimensions?.height || '');
        setWeight(product.weight);
        setMaterial(product.material);
        setColor(product.color);
        setOriginalPackaging(product.original_packaging);
        setManualIncluded(product.manual_included);
        setWorkingConditionDesc(product.working_condition_description);
  ...
  const submit = async (e) => {
    ...
    if (result) {
      window.dispatchEvent(new CustomEvent('hc_toast', { detail: { message: 'Product updated successfully!', type: 'success' } }));
